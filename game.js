...
function addRoadWheelRuts() {
  ...
  [-1.4, 1.4].forEach((offset) => {
    const rut = new THREE.Mesh(
      new THREE.PlaneGeometry(FIELD - PLOT2_X_MIN - 4, rutWidth),
      rutMat
    );
    rut.rotation.x = -Math.PI / 2;
    rut.position.set((PLOT2_X_MIN + FIELD) / 2, 0.02, zC + offset);
    scene.add(rut);
  });
}
addRoadWheelRuts();
...
function addAllFencing() {
  ...
  // - solidny odcinek na ujeżdżalni (od bramy 2 do zachodu)
  ...
  buildFenceSegment(PLOT2_X_MIN, PLAY_Z_MAX, GATE2_X_MIN, PLAY_Z_MAX);            // -90..-70 solidny
}
addAllFencing();
