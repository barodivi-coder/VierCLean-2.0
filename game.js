...
// ---------- Płyta obornikowa: ciemnoszara powierzchnia w kącie między halą a okólnikiem ----------
function addManureSlab() {
  const width = 2.3, depth = 16;
  // w wolnym kącie pomiędzy halą (od wschodu) a okólnikiem (od zachodu/południa),
  // z bezpiecznym zapasem od obu
  const cx = -45.15;
  const cz = -11;

  const border = new THREE.Mesh(new THREE.PlaneGeometry(width + 0.5, depth + 0.5), toonMat(0x2E2C29));
  border.rotation.x = -Math.PI / 2;
  border.position.set(cx, 0.012, cz);
  scene.add(border);

  const slab = new THREE.Mesh(new THREE.PlaneGeometry(width, depth), toonMat(0x4A4A46));
...
function createMiddleWingOpen(width, length, wallHeight, pitchDeg, wallColor, roofColor, doorW, doorH) {
  ...
  addInteriorBand(innerHalfW * 2, 0, 0, -innerHalfL);          // tylna ściana
  addInteriorBand(innerHalfL * 2, Math.PI / 2, -innerHalfW, 0); // lewa ściana
  addInteriorBand(innerHalfL * 2, Math.PI / 2, innerHalfW, 0);  // prawa ściana
...
function createHorseHead() {
  ...
  // szyja — zwężający się walec, u nasady (przy ścianie) szerszy
  const neck = new THREE.Mesh(new THREE.CylinderGeometry(0.24, 0.34, 1.1, 10), skinMat);
  neck.rotation.x = Math.PI / 2;
  neck.position.set(0, -0.06, 0.4);
  ...
  skull.position.set(0, 0.14, 0.95);
...
function addFarmBuilding() {
  ...
  // Drzwi i okna liczone od POPRZEDNIEJ wysokości — podniesienie ścian ich nie zmienia.
  ...
  // ---------- Kolizja ----------
  const bx = PLOT4_CENTER_X, bz = PLAY_Z_CENTER;
  const halfL = length / 2;
