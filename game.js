...
const ROAD_Z_MAX = 32;          // zewnętrzna, północna granica całego świata gry
...
// Brama 1 (łąka -> droga): na północnej krawędzi łąki, w pobliżu startowej
// pozycji traktora — wyjazd niemal na wprost.
const GATE_WIDTH = 10;
const GATE1_X_MIN = -GATE_WIDTH / 2;
const GATE1_X_MAX = GATE_WIDTH / 2;
// Brama 2 (droga -> ujeżdżalnia): na północnej krawędzi plansza-3, wyśrodkowana.
const GATE2_X_MIN = PLOT3_CENTER_X - GATE_WIDTH / 2;
const GATE2_X_MAX = PLOT3_CENTER_X + GATE_WIDTH / 2;
...
// ---------- Asphalt road: runs the FULL width of the game world ----------
// Od wschodniej ściany łąki (x = FIELD) po zachodnią ścianę ujeżdżalni
// (x = PLOT2_X_MIN) — jedna, ciągła droga nad wszystkimi trzema planszami.
function addRoadStrip() {
  const roadWidth = FIELD - PLOT2_X_MIN;   // wzdłuż X — cała szerokość świata
  ...
  road.position.set((PLOT2_X_MIN + FIELD) / 2, 0.015, (ROAD_Z_MIN + ROAD_Z_MAX) / 2);
  road.receiveShadow = true;
  scene.add(road);
}
addRoadStrip();

// Przerywana, jaśniejsza linia na środku drogi — czysto kosmetyczny akcent.
function addRoadCenterLine() {
  const dashMat = toonMat(0xD9D9D0);
  const zC = (ROAD_Z_MIN + ROAD_Z_MAX) / 2;
  const xStart = PLOT2_X_MIN + 3;
  const xEnd = FIELD - 3;
  const dashLen = 2.2, gap = 2.2;
  for (let x = xStart; x < xEnd; x += dashLen + gap) {
    const dash = new THREE.Mesh(new THREE.BoxGeometry(dashLen, 0.02, 0.35), dashMat);
    dash.position.set(x + dashLen / 2, 0.02, zC);
    scene.add(dash);
  }
}
addRoadCenterLine();
...
function addAllFencing() {
  ...
  buildFenceSegment(PLOT2_X_MIN, PLAY_Z_MIN, PLOT2_X_MIN, ROAD_Z_MAX);  // zachód (x = -90)

  // Południowa krawędź (front wszystkich 3 plansz), bez bram.
  buildFenceSegment(PLOT2_X_MIN, PLAY_Z_MIN, FIELD, PLAY_Z_MIN);

  // Zewnętrzna, północna krawędź drogi (prawdziwa granica świata), bez bram.
  buildFenceSegment(PLOT2_X_MIN, ROAD_Z_MAX, FIELD, ROAD_Z_MAX);

  // Wewnętrzne działki (łąka|hala, hala|ujeżdżalnia) — tylko na wysokości
  // plansz (nie wchodzą na drogę), bez bram — przejazd wyłącznie drogą.
  ...
  // Granica plansze <-> droga (z = PLAY_Z_MAX), z DWIEMA bramami:
  // brama 1 (łąka) i brama 2 (ujeżdżalnia); reszta (w tym cała plansza-2/hala) solidna.
  const gates = [
    { from: GATE2_X_MIN, to: GATE2_X_MAX },
    { from: GATE1_X_MIN, to: GATE1_X_MAX }
  ].sort((a, b) => a.from - b.from);

  let cursor = PLOT2_X_MIN;
  gates.forEach((g) => {
    if (g.from > cursor) buildFenceSegment(cursor, PLAY_Z_MAX, g.from, PLAY_Z_MAX);
    buildFenceSegment(g.from, PLAY_Z_MAX, g.to, PLAY_Z_MAX, { from: 0, to: 1 });
    cursor = g.to;
  });
  if (cursor < FIELD) buildFenceSegment(cursor, PLAY_Z_MAX, FIELD, PLAY_Z_MAX);
}
addAllFencing();
...
function updateTractor(dt) {
  ...
  newX = THREE.Math.clamp(newX, PLOT2_X_MIN + M, FIELD - M);
  ...
  // Granica plansze <-> droga (z = PLAY_Z_MAX): przenikalna TYLKO w oknach
  // dwóch bram, i tylko gdy gracz je otworzył.
  const gateWindows = gateOpen
    ? [{ from: GATE1_X_MIN, to: GATE1_X_MAX }, { from: GATE2_X_MIN, to: GATE2_X_MAX }]
    : [];
