...
// ---------- Płyta obornikowa: mała szara powierzchnia obok okólnika ----------
function addManureSlab() {
  const width = 7, depth = 9;
  // tuż obok okólnika (ten sam x co jego środek), między okólnikiem a drogą,
  // z zapasem od hali, dzielnika i samego okólnika
  const cx = -52.5;
  const cz = -8;

  const border = new THREE.Mesh(new THREE.PlaneGeometry(width + 0.6, depth + 0.6), toonMat(0x8A8A85));
  border.rotation.x = -Math.PI / 2;
  border.position.set(cx, 0.012, cz);
  scene.add(border);

  const slab = new THREE.Mesh(new THREE.PlaneGeometry(width, depth), toonMat(0xBFBFB9));
...
function createGableWing(width, length, wallHeight, pitchDeg, wallColor, roofColor, doorWFrac, doorHFrac) {
  ...
  if (doorWFrac) {
    const doorW = Math.min(width * doorWFrac, width * 0.94);
    const doorH = Math.min(wallHeight * doorHFrac, wallHeight * 0.95);
...
function createLeanToWing(width, length, innerHeight, pitchDeg, wallColor, roofColor, innerSide, doorWFrac, doorHFrac) {
  const g = new THREE.Group();
  const halfW = width / 2;
  const pitchRad = THREE.Math.degToRad(pitchDeg);
  ...
  const fillFront = new THREE.Mesh(fillGeo, wallMat);
  fillFront.position.set(0, outerHeight, length / 2);
  fillFront.castShadow = true;
  g.add(fillFront);
  const fillBack = new THREE.Mesh(fillGeo, wallMat);
  fillBack.position.set(0, outerHeight, -length / 2);
  fillBack.rotation.y = Math.PI;
  ...
  if (doorWFrac) {
    const doorW = Math.min(width * doorWFrac, width * 0.85);
    const doorH = Math.min(outerHeight * doorHFrac, outerHeight * 0.9);
    const door = new THREE.Mesh(new THREE.PlaneGeometry(doorW, doorH), toonMat(0x3B2E22));
    door.position.set(0, doorH / 2, length / 2 + 0.03);
    g.add(door);
  }
...
function createHorseHead() {
  ...
  const snout = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.27, 0.5, 8), skinMat);
  snout.rotation.x = Math.PI / 2;
  snout.position.set(0, -0.08, 0.5);
  g.add(snout);

  const earGeo = new THREE.ConeGeometry(0.1, 0.26, 6);
  const earL = new THREE.Mesh(earGeo, skinMat);
  earL.position.set(-0.17, 0.4, -0.1);
  earL.rotation.x = -0.3;
  g.add(earL);
  const earR = new THREE.Mesh(earGeo, skinMat);
  earR.position.set(0.17, 0.4, -0.1);
  earR.rotation.x = -0.3;
  g.add(earR);

  const mane = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.32, 0.65), maneMat);
  mane.position.set(0, 0.33, -0.1);
  g.add(mane);

  return g;
}

// Dodaje 6 niewielkich okienek z wystającymi głowami koni na zewnętrznej,
// dłuższej ścianie skrzydła (skierowanej lokalnie w +X).
function addStableWindows(wingGroup, width, length, wallH) {
  const halfW = width / 2;
  const winY = wallH * 0.62;
  const count = 6;
  ...
  for (let i = 0; i < count; i++) {
    ...
    const hole = new THREE.Mesh(new THREE.PlaneGeometry(0.5, 0.55), toonMat(0x1E1B16));
    hole.position.set(halfW + 0.02, winY, z);
    hole.rotation.y = Math.PI / 2;
    wingGroup.add(hole);

    const head = createHorseHead();
    head.rotation.y = Math.PI / 2;
    head.position.set(halfW + 0.12, winY, z);
...
function addFarmBuilding() {
  ...
  const pitchDeg = 10;
  const wallHeight = 4 * 1.5;   // o połowę wyższe niż poprzednio (6)
  ...
  const left = createLeanToWing(sideWidth, length, wallHeight, pitchDeg, wallColor, roofColor, 1, 0.5, 0.75);
  left.position.x = -(midWidth / 2 + sideWidth / 2);
  building.add(left);

  // środkowa część — dwuspadowy dach, dużo większy otwór drzwiowy
  const middle = createGableWing(midWidth, length, wallHeight, pitchDeg, wallColor, roofColor, 0.82, 0.92);
  building.add(middle);

  // prawe skrzydło (od strony ujeżdżalni) — jednospadowy dach + okienka z końmi
  const right = createLeanToWing(sideWidth, length, wallHeight, pitchDeg, wallColor, roofColor, -1, 0.5, 0.75);
  right.position.x = midWidth / 2 + sideWidth / 2;
  addStableWindows(right, sideWidth, length, right.userData.outerHeight);
  building.add(right);
