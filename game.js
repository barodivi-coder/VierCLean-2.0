...
// ---------- Płyta obornikowa: betonowa powierzchnia wypełniająca narożnik obok okólnika ----------
function addManureSlab() {
  const width = 11, depth = 24;
  // wypełnia niemal cały wolny narożnik plansza-2 (między dzielnikiem, halą
  // i okólnikiem), bez stykania się z żadnym z nich
  const cx = -52.5;
  const cz = -2;
...
function addStableWindows(wingGroup, width, length, wallH) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // głowa konia wystająca z okna
    const head = createHorseHead();
    head.rotation.y = Math.PI / 2;
    head.position.set(halfW + 0.2, winY, z);
