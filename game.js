...
function addCampfirePit() {
  ...
  // płytka, okrągła niecka (Lathe — profil obrócony wokół osi Y)
  const profile = [
    new THREE.Vector2(0, -0.22),
    new THREE.Vector2(radius * 0.7, -0.18),
  ...
  // skrzyżowane, przypalone polana na środku
  const logMat = toonMat(0x3B2A1E);
  [0.5, -0.5].forEach((rot) => {
    const log = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.07, radius * 1.1, 6), logMat);
    log.rotation.z = Math.PI / 2;
    log.rotation.y = rot;
    log.position.set(cx, 0.02, cz);
    scene.add(log);
  });
}
addCampfirePit();
