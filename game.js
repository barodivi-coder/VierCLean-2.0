...
// ---------- Meadow fence (with gate on the left side, bottom-left corner) ----------
function addMeadowFence() {
  ...
  // left side — carries the gate near its bottom-left corner (z close to -half)
  buildFenceSegment(-half, half, -half, -half, { from: 0.82, to: 1 });
}
addMeadowFence();

// ---------- Second plot: "ujeżdżalnia" (concrete yard + red arch hall) ----------
...
const PLOT2_CENTER_X = (PLOT2_X_MIN + PLOT2_X_MAX) / 2;
...
function addPlot2Ground() {
  ...
  const plane = new THREE.Mesh(
    new THREE.PlaneGeometry(PLOT2_X_MAX - PLOT2_X_MIN, PLOT2_Z_MAX - PLOT2_Z_MIN),
    mat
  );
...
function addPlot2Fence() {
  // only the 3 sides that are NOT shared with the meadow — the shared
  // border at x = PLOT2_X_MAX is already fenced (with the gate) by the meadow.
...
function createArchHall(length, width, wallHeight, color) {
  ...
  const ridge = new THREE.Mesh(new THREE.BoxGeometry(length, 0.15, 0.15), toonMat(0x7A2018));
  ridge.position.set(0, wallHeight + r, 0);
  group.add(ridge);
...
function addHall() {
  ...
  // hall starts ~1/3 in from the meadow border, spans the middle third,
  // leaves the far third free — measured from the shared border (PLOT2_X_MAX)
  const hallCenterX = PLOT2_X_MAX - hallLength * 1.5;
  hall.rotation.y = Math.PI / 2;
  hall.position.set(hallCenterX, 0, 0);
  scene.add(hall);
}
addHall();
...
function updateTractor(dt) {
  ...
  const minX = gateOpen ? (PLOT2_X_MIN + 2.2) : -(FIELD - 2.2);
  tractor.position.x = THREE.Math.clamp(tractor.position.x, minX, FIELD - 2.2);
  tractor.position.z = THREE.Math.clamp(tractor.position.z, -marginZ, marginZ);
...
function playUnlockSequence() {
  UNLOCK_LINES.forEach((line, i) => {
    setTimeout(() => showSpeech(line), i * 3600);
  });
  setTimeout(openGate, UNLOCK_LINES.length * 3600);
}
