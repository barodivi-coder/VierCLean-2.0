...
function updateMarten(dt) {
  ...
  const nearMissRadius = 3.6; // próg, przy którym kuna "w ostatniej chwili" umyka zamiast dać się złapać

  if (martenState === 'hala') {
    // W hali złapanie jest fizycznie niemożliwe — gdy traktor się zbliży
    // na tyle, że złapanie byłoby możliwe, kuna zawsze zdąży sprytnie
    // wybiec na otwarty parking (niezależnie od tego, gdzie akurat jest).
    if (distToTractor < nearMissRadius) {
      martenState = 'parking';
      martenMesh.position.set(PLOT4_X_MIN - 3, 0, PLAY_Z_MAX - 5);
      martenWanderTimer = 0;
    }
  } else if (distToTractor < catchRadius) {
    catchMarten();
    return;
  }
