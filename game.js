...
function addHills() {
  ...
  for (let i = 0; i < 34; i++) {
    ...
    const x = worldCenterX + Math.cos(angle) * distX;
    const z = worldCenterZ + Math.sin(angle) * distZ;
...
function addOfficeWindowsAndDoor(wingGroup, width, length, wallH) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (isDoor) {
      ...
      // fragment "wypełniający" pod kalenicą dachu głównego skrzydła — żeby
      // zadaszenie sprawiało wrażenie ciągłego przedłużenia dachu, a nie
      // osobnej, doklejonej konstrukcji
      const filler = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.09, canopyWidth), canopyRoofMat);
      filler.position.set(innerX + 0.2, topY, z);
...
function createVanBus() {
  ...
  // przednia szyba
  const windshield = new THREE.Mesh(new THREE.BoxGeometry(2.05, 0.7, 0.1), glassMat);
  windshield.position.set(0, 1.55, 2.55);
  g.add(windshield);

  // pas okien wzdłuż obu boków
  [-1.13, 1.13].forEach((x) => {
    const sideWin = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.5, 3.2), glassMat);
    sideWin.position.set(x, 1.6, 0.3);
    g.add(sideWin);
  });

  // dolna listwa/zderzak
  ...
  [[-1.05, 1.7], [1.05, 1.7], [-1.05, -1.7], [1.05, -1.7]].forEach(([x, z]) => {
...
function addParkedVan(centerX, centerZ) {
  const van = createVanBus();
  van.position.set(centerX, 0, centerZ);
  scene.add(van);
  FARM_BLOCKERS.push({ x1: centerX - 1.15, x2: centerX + 1.15, z1: centerZ - 2.65, z2: centerZ + 2.65 });
}
// stoją równolegle do największego budynku (długość wzdłuż Z, bez rotacji)
addParkedVan(-124, -8);
addParkedVan(-127.5, -8);
...
function createFireEffect() {
  const g = new THREE.Group();
  const layers = [
    { color: 0x8B2E1F, r: 0.85, h: 1.1, y: 0.4 },
    { color: 0xB3352A, r: 0.68, h: 1.05, y: 0.62 },
    { color: 0xE4572E, r: 0.48, h: 0.95, y: 0.85 },
    { color: 0xFFC145, r: 0.28, h: 0.75, y: 1.08 }
  ];
  const cones = [];
  layers.forEach((l) => {
    const cone = new THREE.Mesh(new THREE.ConeGeometry(l.r, l.h, 10), toonMat(l.color));
    cone.position.y = l.y;
    g.add(cone);
    cones.push(cone);
  });
  ...
  const sparks = [];
  for (let i = 0; i < 8; i++) {
    const spark = new THREE.Mesh(new THREE.SphereGeometry(0.045, 6, 6), sparkMat);
    spark.userData.seed = Math.random() * 10;
    spark.userData.xOff = (Math.random() * 2 - 1) * 0.7;
    spark.userData.zOff = (Math.random() * 2 - 1) * 0.7;
    g.add(spark);
    sparks.push(spark);
  }
...
function igniteBags() {
  ...
  // szerszy w X/Z, żeby wyraźnie obejmować sobą oba worki (rozstawione o ±0.95)
  fireGroup.scale.set(1.4, 1.25, 1.4);
...
function updateFire(dt, t) {
  ...
  fireGroup.userData.cones.forEach((c, i) => {
    const wob = Math.sin(t * 7 + i * 1.7);
    c.scale.y = 1 + wob * 0.28;
    c.scale.x = c.scale.z = 1 + wob * 0.14;
    c.rotation.y += dt * (1.0 + i * 0.35);
  });
