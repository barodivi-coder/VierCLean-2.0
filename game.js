...
function createMiddleWingOpen(width, length, wallHeight, pitchDeg, wallColor, roofColor, doorW, doorH) {
  ...
  const vaultingHorse = createVaultingHorse(1.6, 3.2, 3.2, 0.7);
...
function addOfficeWindowsAndDoor(wingGroup, width, length, wallH) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (isDoor) {
      ...
      // Niewielkie czerwone zadaszenie na brązowych słupkach, ze spadkiem
      // w stronę parkingu (czyli w stronę -X, na zewnątrz od ściany).
      const canopyDepth = 1.15;
      const innerX = -(halfW + 0.08);
      const outerX = innerX - canopyDepth;
      const topY = doorH + 0.35;
      const outerY = topY - 0.45;
      const canopyPostMat = toonMat(0x6B4226);
      const canopyRoofMat = toonMat(0xB3352A);

      [-1, 1].forEach((s) => {
        const post = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, outerY, 6), canopyPostMat);
        post.position.set(outerX + 0.12, outerY / 2, z + s * (doorW / 2 - 0.1));
        post.castShadow = true;
        wingGroup.add(post);
      });
      ...
      const canopyRoof = new THREE.Mesh(new THREE.BoxGeometry(canopyLen, 0.07, doorW + 0.3), canopyRoofMat);
...
// Garaż 2: naprzeciwko pojedynczych drzwi hali 2 (od strony parkingu) —
// identyczny jak garaż 1, ale brama zwrócona z powrotem w stronę tego
// wejścia (na wschód).
addGarage(-137, -16, Math.PI / 2);
...
function updateMarten(dt) {
  ...
  if (distToTractor < fleeRadius) {
    ...
    // losowe błądzenie po hali, ze zmianą kierunku co jakiś czas
  ...
  const margin = 1.3; // żeby kuna nie "wchodziła" w ściany hali
  let nx = martenMesh.position.x + dirX * martenSpeed * dt;
  let nz = martenMesh.position.z + dirZ * martenSpeed * dt;
  nx = THREE.Math.clamp(nx, HALA2_BOUNDS.xMin + margin, HALA2_BOUNDS.xMax - margin);
  nz = THREE.Math.clamp(nz, HALA2_BOUNDS.zMin + margin, HALA2_BOUNDS.zMax - margin);
  martenMesh.position.x = nx;
  martenMesh.position.z = nz;
  martenMesh.rotation.y = Math.atan2(dirX, dirZ);
}

const MARTEN_INTRO_LINE = ['Pan Janusz: Ojej, kuna wlazła do hali! Nie ucieknie stąd - musisz ją złapać traktorem!'];
...
function createFireEffect() {
  const g = new THREE.Group();
  const layers = [
    { color: 0xB3352A, r: 0.32, h: 0.55, y: 0.2 },
    { color: 0xE4572E, r: 0.24, h: 0.5, y: 0.32 },
    { color: 0xFFC145, r: 0.15, h: 0.4, y: 0.46 }
  ];
  const cones = [];
  layers.forEach((l) => {
    const cone = new THREE.Mesh(new THREE.ConeGeometry(l.r, l.h, 8), toonMat(l.color));
    cone.position.y = l.y;
    g.add(cone);
    cones.push(cone);
  });
...
function igniteBags() {
  ...
  spawnedBigBags.forEach((bag) => {
    const flame = createFireEffect();
    flame.position.set(bag.position.x, 0.85, bag.position.z);
    fireGroup.add(flame);
  });
  scene.add(fireGroup);
  fireActive = true;
...
function updateFire(dt, t) {
  ...
  fireGroup.children.forEach((flame, fi) => {
    flame.userData.cones.forEach((c, i) => {
      const wob = Math.sin(t * 7 + i * 1.7 + fi * 2.1);
      c.scale.y = 1 + wob * 0.2;
      c.scale.x = c.scale.z = 1 + wob * 0.1;
      c.rotation.y += dt * (1.2 + i * 0.4);
    });
  });
}
