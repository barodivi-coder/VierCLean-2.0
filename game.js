...
function createMiddleWingOpen(width, length, wallHeight, pitchDeg, wallColor, roofColor, doorW, doorH) {
  ...
  function createVaultingHorse(lowLegH, highLegH, benchLen, benchW) {
    ...
    // pochylona belka (grzbiet kozła) — łączy oba końce na różnych wysokościach
    const beamLen = Math.hypot(benchLen, highLegH - lowLegH);
    const angle = Math.atan2(highLegH - lowLegH, benchLen);
    const beam = new THREE.Mesh(new THREE.BoxGeometry(benchW, 0.3, beamLen), woodMat);
    beam.position.set(0, (lowLegH + highLegH) / 2, 0);
    beam.rotation.x = -angle;
    beam.castShadow = true;
    vg.add(beam);
    // miękka "nakładka" na wierzchu belki
    const pad = new THREE.Mesh(new THREE.BoxGeometry(benchW * 0.96, 0.08, beamLen * 0.97), padMat);
    pad.position.set(0, (lowLegH + highLegH) / 2 + 0.19, 0);
    pad.rotation.x = -angle;
    vg.add(pad);
