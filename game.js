...
function createMiddleWingOpen(width, length, wallHeight, pitchDeg, wallColor, roofColor, doorW, doorH) {
  ...
  const vaultingHorse = createVaultingHorse(0.8, 1.6, 3.2, 0.7);
...
// ---------- Garaż na plansza-5 ----------
// Dach dwuspadowy (kalenica wzdłuż X) — jeden spadek od strony drogi, drugi
// od strony końca planszy. Ściana szczytowa od strony parkingu/stajni (wschód)
// ma wizualne (nieprzejezdne) drzwi garażowe.
function addGarageDoorDetail(wingGroup, doorW, doorH, halfLength) {
  const doorMat = toonMat(0x8A8A85);
  const door = new THREE.Mesh(new THREE.PlaneGeometry(doorW, doorH), doorMat);
  door.position.set(0, doorH / 2, halfLength + 0.03);
  wingGroup.add(door);
  const lineMat = toonMat(0x5A5A56);
  const lines = 4;
...
function addGarage() {
  const garageWidthX = 8;  // wschód-zachód (kierunek kalenicy)
  const garageDepthZ = 8;  // północ-południe (kierunek spadków)
  const wallHeight = 3.2;
  const pitchDeg = 20;
  const wallColor = 0xE3B23C; // żółty
  const roofColor = 0xB3352A; // czerwony

  // createGableWing buduje z kalenicą wzdłuż lokalnego Z — obracamy o 90°,
  // żeby kalenica biegła wzdłuż X (spadki wtedy patrzą na północ/południe).
  ...
  // zaczyna się kawałeczek za miejscem, w którym droga staje się w pełni
  // szara (x = -130), blisko drogi ("na wprost")
  const garageEastX = -133;
  const centerX = garageEastX - garageWidthX / 2;
  const centerZ = 13;
