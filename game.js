...
function addManureSlab() {
  const size = 7; // kwadrat, mniejszy niż poprzednio
  // narożnik przy granicy z łąką (x = -30), tuż za halą (na południe od niej)
  const cx = -34.5;
  const cz = -21;
...
function createMiddleWingOpen(width, length, wallHeight, pitchDeg, wallColor, roofColor, doorW, doorH) {
  ...
  function createRoofTruss(spanHalfW, riseH, thickness) {
    ...
    const bottom = new THREE.Mesh(
      new THREE.CylinderGeometry(barR, barR, spanHalfW * 2, 6),
      steelMat
    );
    ...
    const topLen = Math.sqrt(spanHalfW * spanHalfW + riseH * riseH);
    const topAngle = Math.atan2(riseH, spanHalfW);
    const topL = new THREE.Mesh(new THREE.CylinderGeometry(barR, barR, topLen, 6), steelMat);
    topL.rotation.z = topAngle;
    topL.position.set(-spanHalfW / 2, riseH / 2, 0);
    truss.add(topL);
    const topR = new THREE.Mesh(new THREE.CylinderGeometry(barR, barR, topLen, 6), steelMat);
    topR.rotation.z = -topAngle;
    topR.position.set(spanHalfW / 2, riseH / 2, 0);
    truss.add(topR);

    // słupek na kalenicy (środek, łączy pas dolny ze szczytem)
    const king = new THREE.Mesh(new THREE.CylinderGeometry(barR, barR, riseH, 6), steelMat);
    king.position.set(0, riseH / 2, 0);
    truss.add(king);

    // skratowanie w zygzak po obu połaciach
    const segments = 3;
    for (let side = -1; side <= 1; side += 2) {
      for (let i = 0; i < segments; i++) {
        const t0 = i / segments, t1 = (i + 1) / segments;
        const bx0 = side * spanHalfW * t0, by0 = 0;
        const tx1 = side * spanHalfW * (1 - t1), ty1 = riseH * t1;
        const dx = tx1 - bx0, dy = ty1 - by0;
        const len = Math.hypot(dx, dy);
        const bar = new THREE.Mesh(new THREE.CylinderGeometry(barR * 0.7, barR * 0.7, len, 6), steelMat);
        bar.position.set((bx0 + tx1) / 2, (by0 + ty1) / 2, 0);
